 ...
 */
public class Dws_Interaction_SingerFollow_window extends BaseApp {

    public static void main(String[] args) throws Exception {
        new Dws_Interaction_SingerFollow_window().start(
                10023,
                4,
                "Dws_Interaction_SingerFollow_window"
        );
    }
...
//        TODO  1、读取关注歌手事实表数据
        DataStreamSource<String> singerFollowDS = env.fromSource(
                        FlinkSourceUtil.getkafkaSource("BM_DWD_Interaction_Singer_Follow", "Dws_Interaction_SingerFollow_window"),
...
//        TODO   3、设置水位线
        ...
        SingleOutputStreamOperator<SingerFollowEvent> sfeWM = process.assignTimestampsAndWatermarks(
                WatermarkStrategy
                        .<SingerFollowEvent>forBoundedOutOfOrderness(Duration.ofSeconds(3))
                        .withTimestampAssigner((event, timestamp) -> event.getUpdateTime())
        );

//        TODO   4、开一个窗口长度 24小时,滑动步长为 5s滑动窗口——每5秒更新一次
        WindowedStream<SingerFollowEvent, Long, TimeWindow> windowsDS = sfeWM
                .keyBy(SingerFollowEvent::getSingerId)
                .window(SlidingProcessingTimeWindows.of(Time.hours(24), Time.seconds(5)))
        ...
        SingleOutputStreamOperator<SingerFollowEvent> result = windowsDS.reduce(
                new ReduceFunction<SingerFollowEvent>() {
                    ...
                    public SingerFollowEvent reduce(SingerFollowEvent value1, SingerFollowEvent value2) throws Exception {
                        return SingerFollowEvent
                                .builder()
                                .singerId(value1.getSingerId())
                                .singerName(value1.getSingerName())
                                .singerNationality(value1.getSingerNationality())
                                .singerGender(value1.getSingerGender())
                                .followCount(value1.getFollowCount() + value2.getFollowCount())
                                .build();
                    }
                },
...
//        Dws_Interaction_SingerFollow_window.SingerFollowEvent(singerName=夜鹿(yorushika), singerId=3001, singerNationality=日, singerGender=团体, followCount=17223, windowStart=2025-06-01 00:01:00, windowEnd=2025-06-02 00:01:00, updateTime=1748793659160, partitionDate=2025-06-02)


//        TODO   6、将结果转化jsonStr在写入到 Doris中去
        result
                .map(new DorisMapFunction<>())
                .sinkTo(FlinkSinkUtil.getDorisSink("B1ueMusic.Dws_Interaction_SingerFollow_window"));

//        TODO  7、启动程序
        env.execute("Dws_Interaction_SingerFollow_window");
    }
